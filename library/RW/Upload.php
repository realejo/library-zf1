...
class RW_Upload
{
    ...
    static public function getAssetsReservedRoot()
    {
        ...
        if ( !defined('APPLICATION_DATA')  || realpath(APPLICATION_DATA) == false) {
            throw new Exception('A pasta raiz do data não está definido em APPLICATION_DATA em RW_Upload::getAssetsReservedRoot()');
        }
        ...
        // Verifica se existe e se tem permissão de escrita
        if (!is_dir($path) || !is_writable($path) )  {
            throw new Exception("A pasta raiz de upload data/assets não existe ou não tem permissão de escrita em RW_Upload::getUploadRoot()'");
        }
    ...
    static public function getAssetsPublicRoot()
    {
        ...
        if ( !defined('APPLICATION_HTTP')  || realpath(APPLICATION_HTTP) == false) {
            throw new Exception('A pasta raiz do public_html não está definido em APPLICATION_HTTP em RW_Upload::getAssetsPublicRoot()');
        }
        ...
        // Verifica se existe e se tem permissão de escrita
        if (!is_dir($path) || !is_writable($path) )  {
            throw new Exception("A pasta raiz de upload public_html/assets não existe ou não tem permissão de escrita em RW_Upload::getUploadRoot()'");
        }
